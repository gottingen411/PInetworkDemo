...
for (i in c(1:10)){
  ...
  df_to_add=read.csv(pathtodata,stringsAsFactors = FALSE) %>% select(.,-1) %>% mutate(.,networkID=my_IDs[i])
...
# function to get data frame for network nodes. The df is formatted such that it can be used with visNetwork  
get_vis_nodes=function(df){
 df1=filter(df,!(df[,2] %in% c("Sort By Title","Sort By Year","Sort By Citations")))
 nodesVis=data.frame(id=unique(c(unique(df1[,1]),unique(df1[,2]))),label=unique(c(unique(df1[,1]),unique(df1[,2]))))
 return(nodesVis)
...
get_vis_edges=function(df){
  df1=filter(df,!(df[,2] %in% c("Sort By Title","Sort By Year","Sort By Citations")))
  edgeVis <- df1 %>% select(.,from=1,to=2) %>% mutate(.,weight=1)
  return(edgeVis) 
...
network_extract=function(ID){
  df1=filter(my_df,!(my_df[,2] %in% c("Sort By Title","Sort By Year","Sort By Citations")) & my_df[,3]==ID)
